...
RUN apt-get -y update && \
    apt-get -y upgrade && \
    apt-get -y install python3-pip python3-venv unixodbc-dev apt-transport-https nodejs && \
    apt-get -y install npm libpq-dev curl unzip zip jq less
#
# Update node to 16 from NodeSource because aws-cdk >=2.28.0 requires V16+
